...
def translate_deck(en_path: Path, client: anthropic.Anthropic, accept_reviewed: bool) -> dict:
    ...
    for cell in en_nb["cells"]:
        ...
        is_front = cid is not None and cid == frontmatter_id
        ...
        if prev is None:
            try:
                es_text = do_translate()
            except TranslationError as e:
                translation_failed(cid, cell, en_text, src_hash, e)
                continue
            meta = {
                "source_sha256": src_hash,
                "machine_sha256": sha(es_text),
                "model": MODEL,
                "translated_at": NOW,
                "needs_review": False,
            }
            out_cells.append(build_md_cell(cid, cell, es_text, meta))
            report["new"].append(cid)
            continue

        prev_meta = (prev.get("metadata") or {}).get("translation") or {}
        prev_es_text = cell_text(prev)
        prev_machine = prev_meta.get("machine_sha256")
        human_edited = prev_machine is None or sha(prev_es_text) != prev_machine
        ...
        if accept_reviewed and prev_meta.get("needs_review"):
            meta = {
                "source_sha256": src_hash,
                "machine_sha256": sha(prev_es_text),
                "model": prev_meta.get("model", MODEL),
                "translated_at": NOW,
                "needs_review": False,
            }
            out_cells.append(build_md_cell(cid, cell, prev_es_text, meta))
            report["accepted"].append(cid)
            continue

        if prev_meta.get("source_sha256") == src_hash:
            out_cells.append(prev)  # English unchanged -> reuse verbatim (protects edits)
